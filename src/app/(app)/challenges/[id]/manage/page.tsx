...
import { deleteCustomProject, extendChallengeDeadline } from "@/lib/services/firebase/projects";
...
export default function ManageChallengePage() {
  ...
  const nudgeMessage = `Hey! I'm doing an iSkipped challenge — "${challenge.title}". Every time we skip a purchase, we save toward this goal together. Join me: ${challengeUrl}`;
  ...
  async function handleExtend(additionalDays: number) {
    if (!user || extending) return;
    setExtending(true);
    try {
      await extendChallengeDeadline(user.uid, challengeId, additionalDays);
  ...
  return (
    <main className="min-h-screen p-4 max-w-lg mx-auto pb-24">
          ...
          {challenge.goalAmount > 0 && (
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                of {formatCurrency(challenge.goalAmount)} goal
              </p>
            </div>
          )}
        </div>
          ...
          return (
            <>
              <p className="text-xs mb-3" style={{ color: "var(--text-secondary)" }}>
                {endLabel
                  ? countdown.isExpired
                    ? `Challenge ended on ${endLabel}.`
                    : `Ends on ${endLabel} — ${countdown.label}.`
                  : "No deadline set. Extend below to add one."}
              </p>
              <div className="flex flex-wrap gap-2">
                {([
                  ...
                  <button
                    ...
                    style={{ background: "var(--bg-surface-3)", border: "1px solid var(--border-default)", color: "var(--green-primary)" }}
                  >
                    {extending ? "..." : label}
                  </button>
                ))}
              ...
              {extendSuccess && (
                <p className="text-xs mt-2 font-semibold" style={{ color: "var(--green-primary)" }}>
                  Deadline extended!
                </p>
              )}
            </>
      ...
      {/* Danger Zone */}
      <section className="rounded-2xl p-4" style={{ background: "var(--bg-surface-2)", border: "1px solid rgba(239,68,68,0.25)" }}>
        <p className="text-xs uppercase tracking-wide font-bold mb-1" style={{ color: "#EF4444" }}>
          Danger Zone
        </p>
        <p className="text-xs mb-3" style={{ color: "var(--text-secondary)" }}>
          Ending this challenge removes it for all participants. Any pledged amounts stay in each member&apos;s jar.
        </p>
        {!endConfirm ? (
          <button
